...
// Virtual disk storage (using SPIFFS)
#define VIRTUAL_DISK_SIZE (1024 * 1024 * 2)  // 2MB virtual disk
...
static int32_t onWrite(uint32_t lba, uint32_t offset, uint8_t *buffer, uint32_t bufsize) {
  // Write to SPIFFS-backed virtual disk
  File diskFile = SPIFFS.open("/virtual_disk.img", "r+");
  if (!diskFile) {
    // Create virtual disk file if it doesn't exist
    diskFile = SPIFFS.open("/virtual_disk.img", "w");
    if (diskFile) {
      // Initialize with zeros
      uint8_t zeros[SECTOR_SIZE] = {0};
      for (int i = 0; i < VIRTUAL_DISK_SIZE / SECTOR_SIZE; i++) {
        diskFile.write(zeros, SECTOR_SIZE);
        if (i % 50 == 0) {
          yield(); // Allow other tasks to run
        }
      }
      diskFile.close();
      diskFile = SPIFFS.open("/virtual_disk.img", "r+");
    }
  }
  
  if (!diskFile) {
    Serial.println("Failed to open virtual disk");
    return -1;
  }
  
  // Seek to correct position and write
  size_t pos = (lba * SECTOR_SIZE) + offset;
  diskFile.seek(pos);
  ...
  // Serial.printf("Write LBA: %lu, offset: %lu, size: %lu, written: %zu\n", 
  //               lba, offset, bufsize, written);
...
static int32_t onRead(uint32_t lba, uint32_t offset, void *buffer, uint32_t bufsize) {
  File diskFile = SPIFFS.open("/virtual_disk.img", "r");
  if (!diskFile) {
    Serial.println("Virtual disk not found");
  ...
  size_t pos = (lba * SECTOR_SIZE) + offset;
  diskFile.seek(pos);
  size_t bytesRead = diskFile.read((uint8_t*)buffer, bufsize);
  diskFile.close();
...
void setup() {
  ...
  Serial.printf("SPIFFS Total: %zu bytes\n", SPIFFS.totalBytes());
  Serial.printf("SPIFFS Used: %zu bytes\n", SPIFFS.usedBytes());

  // Initialize virtual disk file if needed
  if (!SPIFFS.exists("/virtual_disk.img")) {
    Serial.println("Creating virtual disk...");
    File diskFile = SPIFFS.open("/virtual_disk.img", "w");
    if (diskFile) {
      uint8_t zeros[SECTOR_SIZE] = {0};
      for (int i = 0; i < VIRTUAL_DISK_SIZE / SECTOR_SIZE; i++) {
        diskFile.write(zeros, SECTOR_SIZE);
        if (i % 100 == 0) {
          Serial.printf("Creating virtual disk: %d%%\n", (i * 100) / (VIRTUAL_DISK_SIZE / SECTOR_SIZE));
        }
      }
      diskFile.close();
      Serial.println("Virtual disk created");
    } else {
      Serial.println("Failed to create virtual disk");
      return;
    }
  }
  ...
  Serial.printf("Virtual disk size: %d sectors of %d bytes = %d MB\n", 
  ...
  Serial.println("Place files on the USB drive - they will be automatically copied to SMB share");
}
...
void loop() {
  ...
  if (WiFi.status() == WL_CONNECTED && !smbConnected) {
    ...
    if (smbClient.begin(SMB_SERVER_IP, SMB_SHARE_NAME, SMB_SHARE_PATH, SMB_USERNAME, SMB_PASSWORD)) {
      ...
      // Non-blocking delay
      static unsigned long smbRetryTime = 0;
      smbRetryTime = millis() + 5000;
      while (millis() < smbRetryTime) {
        handleServer();
        yield();
        delay(10);
      }
  ...
  // Check for new files to copy to SMB (only if WiFi and SMB connected and not already copying)
  if (AUTO_COPY_ENABLED && WiFi.status() == WL_CONNECTED && smbConnected && !copyInProgress) {
    if (millis() - lastCopyCheck > COPY_CHECK_INTERVAL) {
